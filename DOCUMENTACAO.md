...
4. npm i ou npm start  -> Instala todas as bibliotecas necessárias do node.
...
## OBS: 
1. Em react native, as propiedades também são conhecidas como props. `UMA BOA PRÁTICA.`
2. Quando você criar a estilização através do StyleSheet, a constante deverá estar no padrão cammelCase. `UMA BOA PRÁTICA.`
3. Na customização do styles, será necessário deixar uma vírgula após cada seletor e após cada propriedade dentro do seletor. `UMA BOA PRÁTICA.`
...
secureTextEntry={true} -> é uma prop que oculta o texto digitado, ideal para campo de senha.